...
# 取得網路遮罩
get_mask() {
	MASK=$($ListIPcmd $NetCard | grep 'Mask:' | grep -v '127.0.0.1' | awk '{print $4}' | awk -F: '{print $2}')
}

# 取得主機所在網段的網路代表號
get_netip() {
	IP=$($ListIPcmd $NetCard | grep 'inet addr:' | grep -v '127.0.0.1' | awk '{print $2}' | awk -F: '{print $2}')
